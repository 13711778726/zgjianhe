...
class wxconfig
{
	...
	*/
	...
	public function createMenu($data){
		...
		//\Think\Log::record('access_token' . $this->access_token);
		
		$result = $this->http_post(self::API_URL_PREFIX.self::MENU_CREATE_URL.'access_token='.$this->access_token,urldecode(json_encode($data)));
		if ($result)
		{
			$json = json_decode($result,true);
	 ...
	 * 微信用户授权
	 * action    授权之后跳转页面功能
	 * modle_name   不同代理商对应的模块
	 * scope    获取用户头像
	 * t_url    授权成功之后跳转链接
	 * param   跳转链接传递的参数
	 */
	public function authorize($action, $modle_name, $scope= 0, $t_url = "", $param="", $accredit_uri="") {
		
		if (!empty($t_url)) {
			$t_url =urlencode($t_url);
		}
		
		if (empty($accredit_uri)) {
			$accredit_uri = "http://t.poo168.cn/webmodle/";;
		}
				
		$accredit_uri = urlencode($accredit_uri . $modle_name . "/user/get_authorize?action=" . $action . $param .  "&url=" . $t_url);
		...
		$url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" .$this->appid . "&redirect_uri=" . $accredit_uri . "&response_type=code&scope=" . $scope . "&state=poo#wechat_redirect";
	 ...
	 * 获取微信用户信息
	 */
	public function get_wxuserinfo($openid, $access_token) {
	...
	*/
	...
	public function  custom_send($data) {
